    ...
    public function onRender(\Simplify\Form\Action $action, $data, $index)
    {
        ...
        if (isset($data[$this->getName()])) {
            foreach ($data[$this->getName()] as $_index => $_row) {
                $__index = array_merge((array) $index, array(
                    $this->getName(),
                    $_index
                ));
                ...
                while ($elements->valid()) {
                    ...
                    $element->allowEdit = $this->allowEdit && $element->allowEdit;
                    
                    $line['elements'][] = array(
                        'label' => $element->getLabel(),
                        'controls' => $element->onRender($action, $_row, array(
                            $index,
                            $this->getName(),
                            $_index
                        ))
                ...
                $this->onRenderRow($line, $data[$this->getName()][$_index], array(
                    $index,
                    $this->getName(),
                    $_index
                ));
        ...
        while ($elements->valid()) {
            ...
            $dummy['elements'][] = array(
                'label' => $element->getLabel(),
                'controls' => $element->onRender($action, $dummy, array(
                    $index,
                    $this->getName(),
                    'dummy'
                ))
        ...
        $this->set('sortableServiceUrl', $action->form->url()
            ->set('formAction', 'services')
            ->set('serviceName', $this->getName())
            ->set('serviceAction', self::SERVICE_ACTION_SORT));
